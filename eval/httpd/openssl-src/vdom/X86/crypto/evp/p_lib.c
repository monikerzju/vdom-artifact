...
// #define LOG_E printf("[openssl-p_lib.c] Enter: %s\n", __func__);
#define LOG_E

/* Structs for vkeys */
__thread bool vkey_state[VKEY_PAGE_NUM]; /* false -> lock; true -> unlock */
__thread bool is_initialized_local = false;
__thread vkru_t *vkru;

bool vkey_map[VKEY_PAGE_NUM];
void * vkey_mem_root;
pthread_mutex_t vkey_lock;
...
int EVP_PKEY_missing_parameters(const EVP_PKEY *pkey)
{
    ...
    if (pkey != NULL) {
        if (pkey->keymgmt != NULL)
            ...
            int tmp = pkey->ameth->param_missing(pkey);
            /* vkeys */
            mpt_lock((EVP_PKEY *)pkey);
            return tmp;
        }
                            ...
                            int selection)
{
    LOG_E
    /* vkeys */
    mpt_unlock((EVP_PKEY *)a);
    mpt_unlock((EVP_PKEY *)b);
...
int EVP_PKEY_set_type(EVP_PKEY *pkey, int type)
{
    ...
    /* vkeys */
    mpt_unlock(pkey);
    int ret = pkey_set_type(pkey, NULL, type, NULL, -1, NULL);
    /* vkeys */
    mpt_lock(pkey);
    return ret;
}
...
int EVP_PKEY_is_a(const EVP_PKEY *pkey, const char *name)
{
    ...
    if (pkey->keymgmt == NULL) {
        ...
        return pkey_type == type;
    }
    EVP_KEYMGMT *tmp = pkey->keymgmt;

    /* vkeys */
    mpt_lock((EVP_PKEY *)pkey);

    return EVP_KEYMGMT_is_a(tmp, name);
}
                                     ...
                                     size_t publen)
{
    ...
    if (pkey != NULL && evp_pkey_is_provided(pkey)) {
        int ret = EVP_PKEY_set_octet_string_param(pkey,
...
void vkey_map_init(void) {
    vkru = vkru_alloc(1);
    printf("[openssl] vkru_alloc() in thread[%d]\n", (int)syscall(SYS_gettid));

    vkey_mem_root = memalign(getpagesize(), VKEY_PAGE_NUM * getpagesize()); // alignment, size
    memset(vkey_mem_root, 0, VKEY_PAGE_NUM * getpagesize());
    printf("[openssl] vkey_mem_root: %lx\n", (unsigned long int)vkey_mem_root);
    int i;
    for(i = 0; i < VKEY_PAGE_NUM; i++) {
        vkey_map[i] = false;
        int vkey = vkey_alloc();
        wrvkru(vkey, VKEY_AD_INIT);
        void *addr = (void *)((unsigned long int)vkey_mem_root + (unsigned long int)getpagesize() * i);
        printf("i: %d, vkey: %d, addr: %lx\n", i, vkey, (unsigned long int)addr);
        vkey_mprotect(addr, getpagesize(), vkey);
    }

    pthread_mutex_init(&vkey_lock, NULL);

    setbuf(stdout, NULL);
}

int vkey_get_unused_vkey(void) {
    int i;
    pthread_mutex_lock(&vkey_lock);
    for(i = 0; i < VKEY_PAGE_NUM; i++) {
        if(vkey_map[i] == false) {
            vkey_map[i] = true;
            // printf("new vkey: %d\n", i + 1);
            pthread_mutex_unlock(&vkey_lock);
            return i + 1;
        }
    }
    pthread_mutex_unlock(&vkey_lock);
...
void vkey_release_vkey(int vkey) {
    int index = vkey - 1;
    pthread_mutex_lock(&vkey_lock);
    vkey_map[index] = false;
    // printf("free vkey: %d\n", vkey);
    pthread_mutex_unlock(&vkey_lock);
}

void * vkey_to_addr(int vkey) {
    int index = vkey - 1;
    return (void *)((unsigned long int)vkey_mem_root + index * getpagesize());
}

int addr_to_vkey(void *addr) {
    int index = (int)(((unsigned long int)addr - (unsigned long int)vkey_mem_root) / getpagesize());
    return index + 1;
}

inline void mpt_unlock(EVP_PKEY *pkey) {
    if(is_initialized_local == false) {
        /* Init vkru */
        vkru = vkru_alloc(1);
        printf("[openssl] vkru_alloc() in thread[%d]\n", (int)syscall(SYS_gettid));
        is_initialized_local = true;
    }
    if(!pkey) return;

    int vkey = addr_to_vkey((void *)pkey);

    if(vkey_state[vkey] == true) {
        return;
    }
    wrvkru(vkey, VKEY_ND); /* nothing disable */
    // printf("[openssl] wrvkru(VKEY_ND), addr: %lx, vkey: %d, tid: %d\n", (unsigned long int)pkey, vkey, (int)syscall(SYS_gettid));

    vkey_state[vkey] = true;
}

inline void mpt_lock(EVP_PKEY *pkey) {
    if(is_initialized_local == false) {
        /* Init vkru */
        vkru = vkru_alloc(1);
        printf("[openssl] vkru_alloc() in thread[%d]\n", (int)syscall(SYS_gettid));
    ...
    if(vkey_state[vkey] == false) {
        return;
    }
    
    wrvkru(vkey, VKEY_AD); /* access disable */
    // printf("[openssl] wrvkru(VKEY_AD), addr: %lx, vkey: %d, tid: %d\n", (unsigned long int)pkey, vkey, (int)syscall(SYS_gettid));

    vkey_state[vkey] = false;
}
...
EVP_PKEY *EVP_PKEY_new(void)
{
    ...
    if(is_initialized == false) {
        ...
        vkey_init(false);
        printf("[openssl] vkey_init(false)\n");

        vkey_map_init();

        is_initialized = true;
        is_initialized_local = true; // main thread: init vkru in vkey_map_init()
    }
    ...
    int vkey = vkey_get_unused_vkey();
    wrvkru(vkey, VKEY_ND);
    EVP_PKEY *ret = (EVP_PKEY *)vkey_to_addr(vkey);
    memset((void *)ret, 0, sizeof(*ret));

    vkey_state[vkey] = true; /* true -> unlock */

    // printf("[openssl] vkey_alloc, addr: %lx, vkey: %d, thread[%d]\n", (long unsigned int)ret, vkey, (int)syscall(SYS_gettid));
 ...
 err:
    ...
    /* vkeys */
    wrvkru(vkey, VKEY_AD);
    vkey_release_vkey(vkey);
    // printf("[openssl] vkey_free, addr: %lx, vkey: %d, thread[%d]\n", (unsigned long int)ret, vkey, (int)syscall(SYS_gettid));
...
void EVP_PKEY_free(EVP_PKEY *x)
{
    ...
    /* vkeys */
    int vkey = addr_to_vkey((void *)x);
    wrvkru(vkey, VKEY_AD);
    vkey_release_vkey(vkey);
    // printf("[openssl] vkey_free, addr: %lx, vkey: %d, thread[%d]\n", (unsigned long int)x, vkey, (int)syscall(SYS_gettid));
}
...
# define EVP_PKEY_TYPE_STRLEN -1
    ...
    int ret = pkey_set_type(pkey, NULL, EVP_PKEY_NONE,
...
EVP_PKEY *EVP_PKEY_dup(EVP_PKEY *pkey)
{
    ...
    // printf("[openssl] call EVP_PKEY_new() in %s\n", __func__);
...
/* EVP_PKEY_free previous position */
                                    ...
                                    const unsigned char *buf, size_t bsize)
{
    ...
    int ret = EVP_PKEY_set_params(pkey, params);
    /* vkeys */
    mpt_lock(pkey);
    return ret;
}
...
int EVP_PKEY_set_params(EVP_PKEY *pkey, OSSL_PARAM params[])
{
    ...
    if (pkey != NULL) {
        if (evp_pkey_is_provided(pkey)) {
            ...
            return evp_keymgmt_set_params(keymgmt, keydata, params);
        }
#ifndef FIPS_MODULE
        /*
...
int EVP_PKEY_get_params(const EVP_PKEY *pkey, OSSL_PARAM params[])
{
    ...
    if (pkey != NULL) {
        if (evp_pkey_is_provided(pkey)) {
            int ret = evp_keymgmt_get_params(pkey->keymgmt, pkey->keydata, params);
            ...
            int ret = evp_pkey_get_params_to_ctrl(pkey, params);
