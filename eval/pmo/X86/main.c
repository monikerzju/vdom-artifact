...
#endif
#ifdef LOWERBOUND
	pkey_set(lowerbound_pkey, VKEY_WD);
#endif
...
#endif
#ifdef LOWERBOUND
			pkey_set(lowerbound_pkey, VKEY_ND);
#endif
			if(flag) {
                switch_count += 2;
...
#endif
#ifdef LOWERBOUND
    pkey_set(lowerbound_pkey, VKEY_AD);
#endif
    if(flag){
        switch_count += 2;
    }
...
#ifdef LOWERBOUND
	lowerbound_pkey = pkey_alloc(0, 0);
	printf("lowerbound pkey: %d\n", lowerbound_pkey);
    for(i = 0; i < PMO_NUM; i++) {
        pkey_mprotect(pmo_pointer_array[i], PMO_SIZE, PROT_READ | PROT_WRITE, lowerbound_pkey);
        printf("pkey %d protect PMO %d, addr: %lx, size: %d\n", lowerbound_pkey, i, (unsigned long int)(pmo_pointer_array[i]), PMO_SIZE);
    }
	pkey_set(lowerbound_pkey, PKEY_DISABLE_ACCESS);
#endif
