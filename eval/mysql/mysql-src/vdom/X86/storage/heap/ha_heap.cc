...
ha_heap::ha_heap(handlerton *hton, TABLE_SHARE *table_arg)
    : handler(hton, table_arg),
      ...
      single_instance(false) {
        static bool is_initialized_global = false;
        if(is_initialized_global == false) {
          vkey_db = vkey_alloc();
          printf("[heap] Global vkey_db allocated: %d, vkey_alloc() in thread[%d]\n", vkey_db, (int)syscall(SYS_gettid));
          is_initialized_global = true;
        }
      }
