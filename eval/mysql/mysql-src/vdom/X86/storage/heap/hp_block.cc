...
uchar *hp_find_block(HP_BLOCK *block, ulong pos) {
  ...
  wrvkru(vkey_db, VKEY_ND); // vkeys VKEY_ND
...
int hp_get_new_block(HP_BLOCK *block, size_t *alloc_length) {
  ...
  *alloc_length = sizeof(HP_PTRS) * i +
  ...
  // vkeys: protect this memory area
  // if (!(root = (HP_PTRS *)my_malloc(hp_key_memory_HP_PTRS, *alloc_length,
  //                                   MYF(MY_WME))))
  //   return 1;

  int my_size = ((int)(*alloc_length)/getpagesize() + 1) * getpagesize();
  root = (HP_PTRS *)memalign(getpagesize(), my_size);
  // printf("getpagesize(): %d, *alloc_length: %ld, my_size: %d\n", getpagesize(), *alloc_length, my_size);
  // printf("[heap] my_malloc HP_PTRS in hp_block.cc, addr: %lx, length: %d in thread[%d]\n", (unsigned long int)root, my_size, (int)syscall(SYS_gettid));
  vkey_mprotect((void *)root, my_size, vkey_db);
  wrvkru(vkey_db, VKEY_ND); // vkeys VKEY_ND
  printf("[heap] vkey_mprotect, addr: %lx, length: %d, vkey: %d\n", (unsigned long int)root, my_size, vkey_db);
