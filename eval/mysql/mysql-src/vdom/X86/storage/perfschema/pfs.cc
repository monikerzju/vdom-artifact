...
static void *pfs_spawn_thread(void *arg) {
  ...
  printf("[pfs] enter 'pfs_spawn_thread', thread: %d\n", (int)syscall(SYS_gettid));

#ifdef VKEYS
  // printf("\nEnter pfs_spawn_thread, thread[%d]\n", (int)syscall(SYS_gettid)); // vkeys
  // printf("  stack_addr: %lx\n", (unsigned long int)typed_arg->stack_addr);
  int vkey = (unsigned long int)typed_arg->vkey;
  printf("In new thread vkey: %d\n", vkey);
  
  vkru_t *vkru = vkru_alloc();
  wrvkru(vkey, VKEY_ND);
#endif
...
int pfs_spawn_thread_vc(PSI_thread_key key, PSI_thread_seqnum seqnum,
  ...
  PFS_thread *parent;
  // printf("Enter pfs_spawn_thread_vc\n"); // vkeys
  ...
  printf("[pfs.cc] call my_thread_create in pfs_spawn_thread_vc\n"); // vkeys
