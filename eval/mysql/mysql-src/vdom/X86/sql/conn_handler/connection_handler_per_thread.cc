...
static void *handle_connection(void *arg) { // vkeys
  ...
  wrvkru(vkey, VKEY_ND); // VKEY_ND
  vkey_mprotect(channel_info->stack_addr, (1093632 / getpagesize() + 1) * getpagesize(), vkey);
...
bool Per_thread_connection_handler::add_connection(Channel_info *channel_info) { // vkeys
  ...
  */
  ...
  int new_stack_size = (1093632 / getpagesize() + 1) * getpagesize();

  void *new_stack_addr = memalign(getpagesize(), new_stack_size);
