...
int my_thread_create(my_thread_handle *thread, const my_thread_attr_t *attr,
...
#ifdef VKEYS
  // vkeys
  // printf("[my_thread.cc] Enter my_thread_create\n");
  // read attr stacksize
  void *stack_addr;
  size_t stack_size;
  ...
  // printf("  start_routine: %lx\n", (unsigned long int)func);

  // if(stack_size == 0) {
    stack_size = 1048576; // 1MB
  // }
  void *new_stack_addr = memalign(getpagesize(), stack_size);
  pthread_attr_setstack((pthread_attr_t*)attr, new_stack_addr, stack_size);
  // pthread_attr_getstack(attr, &stack_addr, &stack_size);

  // printf("New Thread Config:\n");
  // printf("  stack_addr: %lx\n", (unsigned long int)new_stack_addr);
  // printf("  stack_size: %ld\n", stack_size);
  // printf("  start_routine: %lx\n", (unsigned long int)func);

  if(vkru == NULL) {
    vkru = vkru_alloc(); // init vkru
  }
  int vkey = vkey_alloc();
  // printf("[my_thread_create] allocate vkey: %d in thread[%d]\n", vkey, (int)syscall(SYS_gettid));
  vkey_mprotect(new_stack_addr, stack_size, vkey);
  wrvkru(vkey, VKEY_ND);

  *((int *)arg) = vkey; // pass vkey to new thread
