...
__thread bool is_initialized = false;

extern "C" {
static void *handle_connection(void *arg) { // vkeys
  ...
  printf("[conn] enter 'handle_connection', thread: %d, stack_addr: %lx\n", (int)syscall(SYS_gettid), (unsigned long int)channel_info->stack_addr); // vkeys

  if(is_initialized == false) {
    vkru = vkru_alloc(1);
    is_initialized = true;
    printf("[conn] vkru_alloc: %lx, thread[%d]\n", (unsigned long)vkru, (int)syscall(SYS_gettid));
  }
  int vkey = vkey_alloc();
  int i;
  for(i = 1; i < 128; i++) {
    if(i != vkey)
      wrvkru(i, VKEY_AD_INIT);
  }
  wrvkru(vkey, VKEY_ND); // vkeys VKEY_ND
  const int size_2MB = 2097152;
  vkey_mprotect(channel_info->stack_addr, (1093632 / size_2MB + 1) * size_2MB, vkey);

  // printf("[conn] vkey_mprotect, stack addr: %lx, size: %d, vkey: %d\n", (unsigned long int)channel_info->stack_addr, (1093632 / size_2MB + 1) * size_2MB, vkey);
  ...
  if (my_thread_init()) {
    ...
    for(i = 1; i < 128; i++) {
      if(i != vkey)
        wrvkru(vkey, VKEY_ND_INIT); // vkeys VKEY_ND
    }
    vkey_free(vkey);
    vkru_free();
  ...
  for(i = 1; i < 128; i++) {
    if(i != vkey)
      wrvkru(i, VKEY_ND_INIT); // vkeys VKEY_ND
  }
  vkey_free(vkey);
...
my_thread_attr_t new_connection_attrib;
bool is_initialized_attrib = false;

bool Per_thread_connection_handler::add_connection(Channel_info *channel_info) { // vkeys
  int error = 0;
  my_thread_handle id;
  ...
  */
  ...
  //                         handle_connection, (void *)channel_info); // vkeys
  
  // vkeys
  // set stack, protected by vkey
  // pthread_attr_getguardsize(&connection_attrib, &my_guardize);
  // my_thread_attr_getstacksize(&connection_attrib, &my_stack_size);

  if(is_initialized_attrib == false) {
      (void)my_thread_attr_init(&new_connection_attrib);
      my_thread_attr_setdetachstate(&new_connection_attrib, MY_THREAD_CREATE_DETACHED);
      pthread_attr_setscope(&new_connection_attrib, PTHREAD_SCOPE_SYSTEM);
      is_initialized_attrib = true;
  }

  const int size_2MB = 2097152;
  int new_stack_size = (1093632 / size_2MB + 1) * size_2MB;

  void *new_stack_addr = memalign(size_2MB, new_stack_size);
  pthread_attr_setstack((pthread_attr_t*)&new_connection_attrib, new_stack_addr, new_stack_size);
  printf("new stack addr: %lx, stack size: %d\n", (unsigned long int)new_stack_addr, new_stack_size);

  channel_info->stack_addr = new_stack_addr;

  printf("Per_thread_connection_handler::add_connection, call my_thread_create in thread[%d]\n", (int)syscall(SYS_gettid));
  ...
  error = my_thread_create(&id, &new_connection_attrib, handle_connection, (void *)channel_info);
